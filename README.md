...
- `src/pipeline/generation/`: data generators (`generate_people`, `daily_generate_people`).
- `src/pipeline/ingestion/`: loaders (`load_to_snowflake` defaults to `historical_data/landing/people.csv`).
- `src/pipeline/utils/`: helpers (`csv_summary` for quick profiling).
...
export PYTHONPATH=src  # consider adding to your shell rc
```
