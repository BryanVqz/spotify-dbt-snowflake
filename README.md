...
Starter kit for Snowflake + dbt + AWS S3. Everything lives under a single repo with one root venv, dbt project under `dbt/`, Terraform infra under `amazon-pipeline/`, and Snowflake bootstrap SQL under `src/` (to run in a Snowsight worksheet/notebook). Remote Terraform state is already configured for multi-laptop use.

## Layout
- `src/snowflake_bootstrap.sql`: one-time Snowflake setup (role, warehouse, database, schema, user).
